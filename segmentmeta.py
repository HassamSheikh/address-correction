...
singleWordProb = OneGramDist('WordFrequencyFeedMetaphone.txt')
...
def segmentamt(word):
   ...
   allSegmentations = [[first] + segmentamt(rest) for (first,rest) in splitPairs(word)]
   return max(allSegmentations, key = wordSeqFitness)

def splitPairs(word, maxLen=0):
